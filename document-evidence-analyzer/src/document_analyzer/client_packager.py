...
This module creates professional client deliverable packages containing
all case analysis results, visualizations, reports, and evidence catalogs
formatted for legal proceedings and consulting engagements.
"""
...
from .case_summary_generator import CaseSummaryGenerator, CaseSummary
...
class ClientPackager:
    ...
    def __init__(self, evidence_manager: EvidenceManager, openai_client=None):
        ...
        self.summary_generator = CaseSummaryGenerator(evidence_manager, openai_client)
    ...
    def _create_package_structure(self, package_dir: Path):
        ...
        # Create raw evidence directory if needed
        subdirs.append("raw_evidence")
    ...
    def _copy_evidence_analysis(self, evidence_summary, package_dir: Path) -> Optional[str]:
        ...
        try:
            ...
            evidence_dir = self.evidence_manager.derived_dir / f"sha256={evidence_summary.sha256}"
    ...
    def _copy_visualizations(self, case_summary: CaseSummary, package_dir: Path) -> List[str]:
        ...
        for evidence in case_summary.evidence_summaries:
            if evidence.evidence_type == "document":
                ...
                evidence_dir = self.evidence_manager.derived_dir / f"sha256={evidence.sha256}"
    ...
    def _create_evidence_catalog(self, case_summary: CaseSummary, package_dir: Path) -> str:
        ...
        for evidence in case_summary.evidence_summaries:
            evidence_entry = {
                ...
                "chain_of_custody": f"Complete audit trail available in evidence/derived/sha256={evidence.sha256}/chain_of_custody.json"
            }
    ...
    def _create_correlation_report(self, case_summary: CaseSummary, package_dir: Path) -> str:
        ...
        correlation_data = {
            "case_id": case_summary.case_id,
            "analysis_timestamp": case_summary.generation_timestamp.isoformat(),
            "entity_correlations": [
                {
                    ...
                    "evidence_pieces": [occ["evidence_sha256"][:8] for occ in corr.evidence_occurrences]
                }
            ...
            "timeline_events": [
                {
                    ...
                    "evidence_sha256": event.evidence_sha256[:8],
...
"""
    ...
    def _copy_raw_evidence(self, case_summary: CaseSummary, package_dir: Path) -> List[str]:
        ...
        for evidence in case_summary.evidence_summaries:
            try:
                # Find original file
                evidence_dir = self.evidence_manager.raw_dir / f"sha256={evidence.sha256}"
    ...
    def _create_package_metadata(self, case_summary: CaseSummary, components: Dict[str, List[str]]) -> Dict[str, Any]:
        ...
        return {
            "package_info": {
                ...
                "evidence_count": case_summary.evidence_count,
                "evidence_types": case_summary.evidence_types,
                "generator": "Evidence Toolkit v1.0"
            },
            "analysis_summary": {
