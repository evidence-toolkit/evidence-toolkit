...
Return structured JSON with entity groups."""
    ...
    def _resolve_entities_with_ai(
        ...
        # NEW v3.2: Match unmatched singles with each other to bootstrap correlations
        # This handles cases where there are NO existing correlations (all entities are singles)
        unmatched_singles = [s for s in singles if s['name'] not in matched_singles]

        if unmatched_singles and self.verbose:
            print(f"   Checking {len(unmatched_singles)} unmatched singles for cross-matches...")

        # Only compare person entities (skip email addresses, organizations, dates)
        person_singles = [s for s in unmatched_singles if s['type'] == 'person']

        # Limit AI calls to prevent timeout
        max_ai_calls = 50
        ai_calls_made = 0

        for i, single_a in enumerate(person_singles):
            if ai_calls_made >= max_ai_calls:
                if self.verbose:
                    print(f"   ⚠️  Hit AI call limit ({max_ai_calls}), skipping remaining comparisons")
                break

            for single_b in person_singles[i+1:]:
                # Skip if from same evidence (can't be a correlation)
                if single_a['sha256'] == single_b['sha256']:
                    continue

                # Skip if already matched
                if single_a['name'] in matched_singles or single_b['name'] in matched_singles:
                    continue

                # Ask AI if they match
                ai_calls_made += 1
                match_result = self._ai_match_entities(
                    entity_a=single_a['name'],
                    context_a=single_a['context'],
                    type_a=single_a['evidence_type'],
                    entity_b=single_b['name'],
                    context_b=single_b['context'],
                    type_b=single_b['evidence_type']
                )

                if match_result and match_result.is_same_entity and match_result.confidence > 0.7:
                    # Create new correlation from these two singles
                    new_corr = CorrelatedEntity(
                        entity_name=single_a['name'],  # Use first entity's name as canonical
                        entity_type='person',
                        occurrence_count=2,
                        confidence_average=(single_a['confidence'] + single_b['confidence']) / 2,
                        evidence_occurrences=[
                            {
                                'evidence_sha256': single_a['sha256'],
                                'evidence_type': single_a['evidence_type'],
                                'entity_type': single_a['type'],
                                'context': single_a['context'],
                                'confidence': single_a['confidence'],
                                'original_name': single_a['name']
                            },
                            {
                                'evidence_sha256': single_b['sha256'],
                                'evidence_type': single_b['evidence_type'],
                                'entity_type': single_b['type'],
                                'context': single_b['context'],
                                'confidence': single_b['confidence'],
                                'original_name': single_b['name']
                            }
                        ]
                    )
                    new_correlations.append(new_corr)
                    matched_singles.add(single_a['name'])
                    matched_singles.add(single_b['name'])

                    if self.verbose:
                        print(f"   ✓ NEW correlation: '{single_a['name']}' ↔ '{single_b['name']}' (AI confidence: {match_result.confidence:.2f})")
    ...
    def _ai_match_entities(
        ...
        """Use AI to determine if two entities refer to the same real-world person/organization.

        v3.2 feature: Solves correlation gaps where string matching fails
        (e.g., "Paul" vs "Paul Boucherat" in different evidence).

        Args:
            entity_a: First entity name
            context_a: Surrounding text/context for entity A
            type_a: Evidence type (document/email/image)
            entity_b: Second entity name
            context_b: Surrounding text/context for entity B
            type_b: Evidence type

        Returns:
            EntityMatchResult with match decision and confidence, or None if AI unavailable
        """
        if not self.openai_client:
            return None
        ...
        try:
            ...
            # Format prompt with entity details
            prompt = legal_config.ENTITY_MATCH_PROMPT.format(
                entity_a=entity_a,
                context_a=context_a[:500],  # Limit context length
                type_a=type_a,
                entity_b=entity_b,
            ...
            if response.status == "completed" and response.output_parsed:
                ...
                if self.verbose:
                    print(f"⚠️  AI entity matching incomplete for '{entity_a}' vs '{entity_b}'")
                return None
            else:
                # Check for refusal
                if hasattr(response.output_parsed, 'refusal') and response.output_parsed.refusal:
                    if self.verbose:
                        print(f"⚠️  AI refused entity matching: {response.output_parsed.refusal}")
                return None

        except Exception as e:
            if self.verbose:
                print(f"⚠️  AI entity matching failed: {e}")
            return None
