#!/usr/bin/env python3
"""
Document Evidence Analyzer - Text Analysis Module (v3.0)
...
# Import our structured analysis models from unified core
try:
...
# try:
#     from evidence_toolkit.core.validation import DocumentValidator
...
class DocumentAnalyzer:
    ...
    def analyze_directory(self,
        ...
        if self.ai_enabled:
            if self.verbose:
                print(f"\n🤖 Running AI analysis on extracted text...")
            ai_analysis = self.analyze_with_ai(all_text)
            if ai_analysis:
                result['ai_analysis'] = {
                    'summary': ai_analysis.summary,
                    'entities': [entity.model_dump() for entity in ai_analysis.entities],
    ...
    def analyze_text(self,
        ...
        if self.ai_enabled:
            if self.verbose:
                print(f"🤖 Running AI analysis (ai_enabled={self.ai_enabled})...")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python document.py <data_directory>")
